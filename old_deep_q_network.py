...
def createNetwork():
    ...
    xStates = Input(shape=(max_rememberance_length, observation_space_shape))

    # 4 convolution nets in a row

    # network weights
    W_conv1 = weight_variable([4, 4, 4, 8])
...
def trainNetwork(network, readout, h_fc1, sess):
    ...
    x_t = np.pad(x_t, ((5,5), (0,0)), 'constant', constant_values=(0))
    ...
    while "pigs" != "fly":
        ...
        readout_t = readout.eval(feed_dict = {network : [s_t]})[0]
        ...
        if t > OBSERVE:
            ...
            y_batch = []
            readout_j1_batch = readout.eval(feed_dict = {network : s_j1_batch})
